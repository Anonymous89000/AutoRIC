...
def cvx_optimize(netname,params_index,flag):
    ...
    sum = 0
    for j in range(0, 1):
        fairness = util.cal_fairness_age_simply.cal_fairness1(model.copy())
        print(fairness)
        sum += fairness
    sum = sum
    print("优化后的网络公平性:", sum)
    ...
    optimized_acc=util.cal_fairness_age_simply.recal_acc(model, optimized_net)
...
if __name__=='__main__':
    cvx_optimize("bnet24",params_index_c,1)