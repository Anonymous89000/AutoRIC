...
import torch
from torch import nn
import util.cal_fairness_age_simply
...
disturbs_0mul10=[10*i for i in disturbs_0]
...
def quadratic_fit(net,n0,net_type0,params_index,disturbs):
    ...
    p =util.get_fit_data.getpara2(model.copy(), 0.048333)
    ...
    for i,delta in enumerate(disturbs):
        ...
        sum=0
        for j in range(1):
            fainess_tmp=util.cal_fairness_age_simply.cal_fairness1(tmp_model)
            sum+=fainess_tmp
        sum=sum
...
if __name__=="__main__":
    net = 'data/bank.pt'
    nettype='bnet24'
    quadratic_fit(net,24,nettype,params_index_c,disturbs_0mul10)