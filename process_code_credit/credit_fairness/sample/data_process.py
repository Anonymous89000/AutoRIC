...
def calculate_female_acc():
    feature = []
    # income = []
    with open("gender_fairness/sample/female_gen.csv", "r") as ins:
        for line in ins:
            line = line.strip()
            features = line.split(';')
    ...
    np.savetxt("gender_fairness/sample/female_feature.txt", feature)
...
def calculate_male_acc():
    feature = []
    # income = []
    with open("gender_fairness/sample/male_gen.csv", "r") as ins:
        for line in ins:
            line = line.strip()
            features = line.split(';')
            features = features[0].split(',')
    ...
    feature = np.asarray(feature)
    # income = np.asarray(income)
    np.savetxt("gender_fairness/sample/male_feature.txt", feature)


def calculate_all_acc():
    # 读取数据
    all = pd.read_csv(r'gender_fairness/sample/all_gen_s3.csv',)
    all = all.drop(['Unnamed: 0'],axis = 1)
    # print(male.columns)
    object_columns = []
    int_columns = []
    for i in all.columns:
        if all.loc[:,i].dtype == "object":
            object_columns.append(i)
        if all.loc[:,i].dtype == "int64":
            int_columns.append(i)
    # 离散数据
    for col_name in object_columns:
        values = np.array(all[col_name])
        # print(values)
        onehot_encoder = OneHotEncoder(sparse=False)
        values = values.reshape(len(values), 1)
        onehot_matrix = onehot_encoder.fit_transform(values)
        # print(onehot_matrix)
        all.drop([col_name],axis=1,inplace=True)
        # 在 Dataframe 首列插入one-hot后新列
        for i in range(onehot_matrix.shape[1]):
            all.insert(0, 'new_'+col_name+"_"+str(i) , value=onehot_matrix[:,i])
    # 连续数据
    for col_name in int_columns:
        Scaler = MinMaxScaler(feature_range=(-1, 1))
        col_value = np.array(all[col_name]).reshape(-1,1)
        new_col = Scaler.fit_transform(col_value)
        all[col_name] = new_col
    all.to_csv("gender_fairness/sample/all_feature_col.csv")  
    # 转成numpy格式
    feature = all.values
    print(feature.shape)
    np.savetxt("gender_fairness/sample/all_feature_s3.txt",feature)
...
if __name__ == "__main__":
    ...
    test_x = np.loadtxt('gender_fairness/sample/female_feature.txt')# 删减三个特性后数据
