...
class LSTM(Layer):
    ...
    def copy(self):
        """ TODO: complete copy
        """    
        new_layer = LSTM(np.zeros((1,1)), np.zeros(1), np.zeros(1), np.zeros(1))

        new_layer.weights = self.weights.copy()
        new_layer.bias = self.bias.copy()
        new_layer.h_0 = self.h_0.copy()
        new_layer.c_0 = self.c_0.copy()

        new_layer.h_t = self.h_t.copy()
        new_layer.c_t = self.c_t.copy()
        return new_layer
