...
fairness_list_0 = [0.131, 0.141, 0.123, 0.092, 0.112, 0.116,
                   0.125, 0.139, 0.113, 0.164, 0.057, 0.237, 0.052, 0.203]
...
def getpapra2_with_lib_model(model, fairness, params_index):
    """ collect data to make csv file
    """
    p = []
    for i in params_index:
        layer_index, neuron_index = i
...
def modify_lib_model_param(model, params, disturb):
    """ eureka update.
        modify param in custom lib model
    """
    for param in params:
        layer_idx, pos = param[0], param[1] - 1
        layer = model.layers[layer_idx - 1]
        weight = layer.get_weights().T
        weight[pos] += disturb
        layer.weights = weight.T
    return model.copy()
...
def rtest(model, test_x, device):
    tensor_test_x = torch.FloatTensor(
        test_x.copy())  # transform to torch tensor
    test_dataset = TensorDataset(tensor_test_x)  # create dataset
    test_dataloader = DataLoader(
        test_dataset, batch_size=100, shuffle=True)  # create dataloader
